...
// @note To be kept below windows.h
#include <DbgHelp.h>

namespace logging {

void CallStackBackend::refresh(uint32_t discardCount) {
    m_entries.clear();
    ...
    while (StackWalk64(image, process, thread, &stackframe, &context, NULL,
        ...
        VirtualQuery(reinterpret_cast<LPVOID>(stackframe.AddrPC.Offset), &infoMemory,
                     sizeof(infoMemory));
        ...
        const auto moduleName =
            wstrConvert.to_bytes(reinterpret_cast<wchar_t*>(wModuleName));

        // From module name, find line infos
        std::stringstream addr2lineCommand;
        addr2lineCommand << "addr2line -e \"" << moduleName << "\" " << std::hex
                         << stackframe.AddrPC.Offset << std::dec;
        ...
        if (functionName[0] == 'Z') { functionName = "_" + functionName; }
        ...
        Entry entry;
        entry.file =
            fileBuffer.substr(fileSeperator + 1, numberSeparator - fileSeperator);
