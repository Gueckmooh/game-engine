...
void CallStackBackend::refresh(uint32_t discardCount) {
    ...
    for (int i = discardCount + 1; i < stackDepth; i++) {
        ...
        auto reference =
            message.substr(leftParenthesisPos + 1, plusPos - leftParenthesisPos - 1);
        auto relativeOffset =
            message.substr(plusPos + 1, rightParenthesisPos - plusPos - 1);

        if (relativeOffset.empty()) { continue; }

        // Find lib offset
        std::string libOffset("0x0");
        if (reference.size() > 0 && reference != "__libc_start_main") {
            auto nmCommand = "echo `nm \"" + libPath + "\" 2> /dev/null` | grep "
                             + reference + " | cut -d' ' -f 1";
            libOffset = "0x" + executeCommand(nmCommand);

            // If no symbol, the output is "nm: <libname>: no symbol".
            if ((libOffset[2] < '0' || libOffset[2] > '9')
                && (libOffset[2] < 'a' || libOffset[2] > 'f')) {
                libOffset = "0x0";
            }
        }

        auto addr2lineCommand = "addr2line -e \"" + libPath + "\" $(printf \"%X\" $(("
                                + libOffset + " + " + relativeOffset + ")))";
