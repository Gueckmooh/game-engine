...
#define $pimpl_class(Class, ...)                                                         \
    Class::Class($parameter_merge(__VA_ARGS__))                                          \
        : fpImpl(std::make_unique<Impl>($parameter_drop_types(__VA_ARGS__))) {}
#define $pimpl_class_delete(Class)                                                       \
    Class::~Class() {}
#define $pimpl_class_namespace(Namespace, Class, ...)                                    \
    Namespace::Class::Class($parameter_merge(__VA_ARGS__))                               \
        : fpImpl(std::make_unique<Impl>($parameter_drop_types(__VA_ARGS__))) {}
#define $pimpl_class_delete_namespace(Namespace, Class)                                  \
    Namespace::Class::~Class() {}

#define $pimpl_method_definition(Class, type, name, ...)                                 \
    type Class::name($parameter_merge(__VA_ARGS__))
#define $pimpl_method_implementation(Class, type, name, ...)                             \
    $pimpl_method_implementation_return(type)                                            \
        fpImpl->name($parameter_drop_types(__VA_ARGS__));

#define $pimpl_method(...)                                                               \
    $pimpl_method_definition(__VA_ARGS__) { $pimpl_method_implementation(__VA_ARGS__) }

#define $pimpl_method_const(...)                                                         \
    $pimpl_method_definition(__VA_ARGS__) const {                                        \
        $pimpl_method_implementation(__VA_ARGS__)                                        \
    }

#define $pimpl_method_implementation_return(type)                                        \
    $cat($pimpl_method_implementation_return_, $type_void(type))
